...
max_steps_per_episode = 100000  # Adjust as needed
    ...
    def step(self, action):
        ...
        if action == 0:  # Continue driving
            if self.fuel_level > 0.1 and self.tire_tread_level > 0.1:
                ...
                reward -= 20.0  # Negative reward for risking running out
...
# Train the model
# pit_model.learn(total_timesteps=5000)

# Save the trained model
pit_model.save("pitnew")
print("Higher-level RL agent trained and saved as 'pit_stop_rl_agent'")

# Load the trained higher-level RL agent
pit_model = PPO.load("pitnew")
# Set the number of episodes
num_episodes = 5  # Adjust as needed
current_episode = 0
all_episode_rewards = []  # List to keep track of the total rewards for each episode
...
while current_episode < num_episodes:
    ...
    print(f"Starting Episode {current_episode + 1}/{num_episodes}")
    ...
    while not done:
        ...
        for i in range(num_agents):
            ...
            tire_tread_levels[i] = max(
                tire_tread_levels[i] - tire_wear_rate * (1.0 / FPS), 0.0
            )
        ...
        for i in range(num_agents):
            ...
            if pit_action == 1 and step_counter % 100 == 0:
    ...
    all_episode_rewards.append(
        sum(total_rewards)
    )  # Store the sum of total rewards for plotting
    print(f"Episode {current_episode + 1} finished with rewards: {total_rewards}")
    current_episode += 1
