...
class SingleAgentWrapper(gym.Wrapper):
    ...
    def step(self, action):
        # Ensure action is a numpy array and convert to list
...
# Add a pit stop flag and timer for each car
in_pit_stop = [False for _ in range(num_agents)]
pit_stop_timers = [0 for _ in range(num_agents)]
pit_stop_duration = 50  # Adjust as needed
while not done:
    actions = []
    for i in range(num_agents):
        if in_pit_stop[i]:
            # Create zero action
            zero_action = np.zeros_like(agent_envs[i].action_space.sample())
            actions.append(zero_action)
            pit_stop_timers[i] += 1
            if pit_stop_timers[i] >= pit_stop_duration:
                in_pit_stop[i] = False
                pit_stop_timers[i] = 0
                fuel_levels[i] = 1.0
            ...
            pit_action, _ = pit_model.predict(pit_obs.reshape(1, -1))
            ...
            if pit_action == 1:
                ...
                # Get action from the low-level model
                ...
                action = action[0]
                action = np.array(action, dtype=agent_envs[i].action_space.dtype)
                actions.append(action)
                episode_starts[i] = False

    # Combine actions
    combined_actions = np.array([actions[i] for i in range(num_agents)])
    ...
    for i in range(num_agents):
        # Pass the action as a list to the agent's environment
        obs_temp, agent_reward, agent_done, _ = agent_envs[i].step([actions[i]])
        obs[i] = obs_temp[0]
        agent_reward = agent_reward[0]
        agent_done = agent_done[0]
